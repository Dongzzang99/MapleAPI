...
public class kdhController2 {
    ...
    @GetMapping("/kdh")
    public String kdhhtml(){
        return "kdh";
    }

    //캐릭터 닉네임을 검색하면 ocid 값을 받아옴
    @GetMapping("/api/searchCharacter2")
    ...
    @GetMapping("/api/getskillicon")
    //ocid 값을 js 에서 들고오기
    public Mono<ResponseEntity<String>> getSkillIcon(@RequestParam String ocid) {
        // 비동기 GET 요청
        return webClient.get()
                //ocid값을 담은 정보를 api 서버로 보냄
                .uri(NEXON_API_URL + "character/skill?ocid=" + ocid+ "&character_skill_grade=6")
                .header("x-nxopen-api-key", API_KEY)
                .retrieve()
                .bodyToMono(String.class)
                .doOnNext(response -> System.out.println("스킬 정보 값: " + response))
                .map(response -> ResponseEntity.ok(response));
    }

    // /maplestory/v1/character/skill

//    @GetMapping("/api/getskillEnforce")
//    //ocid 값을 js 에서 들고오기
//    public Mono<ResponseEntity<String>> getSkillEnforce(@RequestParam String ocid) {
//        // 비동기 GET 요청
//        return webClient.get()
//                //ocid값을 담은 정보를 api 서버로 보냄
